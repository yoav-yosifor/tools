...
import laser_sweep as _laser_sweep_module
...
def _abspath_redirector(tmp_path):
    """Return a side_effect for os.path.abspath that redirects laser_sweep.py → tmp_path."""
    real_abspath = os.path.abspath
    module_file = _laser_sweep_module.__file__

    def _fake(path):
        if path == module_file:
            return str(tmp_path / "laser_sweep.py")
...
def _run_sweep(ctrl, tmp_path, repetitions=1):
    ...
    with patch("os.path.abspath", side_effect=_abspath_redirector(tmp_path)):
    ...
    return results, folder, str(tmp_path / "results")
...
class TestOutputStructure:
    ...
    def test_single_rep_files_exist(self, sweep1):
        _, folder, _ = sweep1
        assert os.path.isfile(os.path.join(folder, "data", "001.csv"))
        assert os.path.isfile(os.path.join(folder, "plots", "001.png"))

    def test_multi_rep_files_exist(self, ctrl, tmp_path):
        _, folder, _ = _run_sweep(ctrl, tmp_path, repetitions=3)
        for i in ("001", "002", "003"):
            assert os.path.isfile(os.path.join(folder, "data", f"{i}.csv")), f"{i}.csv missing"
            assert os.path.isfile(os.path.join(folder, "plots", f"{i}.png")), f"{i}.png missing"

    def test_no_extra_csv_files(self, ctrl, tmp_path):
        _, folder, _ = _run_sweep(ctrl, tmp_path, repetitions=2)
        csvs = [f for f in os.listdir(os.path.join(folder, "data")) if f.endswith(".csv")]
        assert len(csvs) == 2

    def test_no_extra_png_files(self, ctrl, tmp_path):
        _, folder, _ = _run_sweep(ctrl, tmp_path, repetitions=2)
        pngs = [f for f in os.listdir(os.path.join(folder, "plots")) if f.endswith(".png")]
        assert len(pngs) == 2
...
class TestCSV:
    ...
    def _df(self, sweep1, rep="001"):
        _, folder, _ = sweep1
        return pd.read_csv(os.path.join(folder, "data", f"{rep}.csv"))
    ...
    def test_no_index_column(self, sweep1):
        ...
        _, folder, _ = sweep1
        with open(os.path.join(folder, "data", "001.csv")) as f:
            header = f.readline().strip()
        assert header == "Wavelength_nm,Amplitude_V"

    def test_multiple_reps_same_structure(self, ctrl, tmp_path):
        _, folder, _ = _run_sweep(ctrl, tmp_path, repetitions=2)
        df1 = pd.read_csv(os.path.join(folder, "data", "001.csv"))
        df2 = pd.read_csv(os.path.join(folder, "data", "002.csv"))
...
class TestPlot:
    ...
    def test_file_is_valid_png(self, sweep1):
        _, folder, _ = sweep1
        with open(os.path.join(folder, "plots", "001.png"), "rb") as f:
            header = f.read(8)
    ...
    def test_plotted_y_matches_csv_amplitude(self, captured_ax):
        ...
        df = pd.read_csv(os.path.join(folder, "data", "001.csv"))
